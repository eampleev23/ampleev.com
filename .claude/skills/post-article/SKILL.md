...
- Identity: always use the author's reference photos in `/Users/eampleev/Documents/my_face` for the face, hairstyle, and beard (the folder holds his best angles; render the beard as in the photos). When Codex generates directly, pass these photos as identity references; when Claude writes a handoff prompt, instruct Codex to use them. Do not invent a different face for the author.
