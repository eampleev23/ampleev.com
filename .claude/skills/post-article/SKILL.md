...
- Identity: always use the author's reference photos in `/Users/eampleev/Documents/my_face` for the face, hairstyle, and beard (the folder holds his best angles; render the beard as in the photos). When Codex generates directly, attach these photos as identity-reference inputs to the generation call — a text description alone is not enough; when Claude writes a handoff prompt, instruct Codex to do so. Hair and beard color must match the photos (light-brown hair, reddish-blond beard), not default to generic dark. At review, compare the generated face against the reference photos and reject the image if it reads as a different person or the hair/beard color is off.
- Posture: natural and relaxed — sitting back comfortably in a proper chair, weight settled, shoulders at ease. Reject crouched, perched, hunched, or otherwise anatomically awkward poses.
...
Screen content follows camera logic: show on-screen content only when the screen face is actually visible to the camera. If a display is seen from behind or at a non-viewing angle, render a clean physical rear (no phantom windows or glowing content on the back). Terminal windows, when shown, must match the author's real setup: macOS Terminal with a white/light background and black text — not a dark theme — and content relevant to the article's topic. Plan the parallax title-safe zone: the article title renders near the center of the hero (slightly below center and toward the left), so keep that central zone free of meaningful detail — screens, readable text, and the person belong toward the frame edges, with calm texture and light in the center.
