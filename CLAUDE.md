...
## Прод-доступ для Claude
Владелец разрешил Claude выполнять операционные команды на проде по SSH (алиас `simplecloud`, приложение в `/var/www/ampleev.com/blog`) в следующих границах:
- **Можно без подтверждения**: чтение логов (`storage/logs/`, `storage/yai/chats/`), `php artisan`-команды (config:cache, yai:build-corpus и т.п.), правка **несекретных** строк `.env` (URL провайдеров, имена моделей, флаги) через sed.
- **Нельзя никогда**: работать со значениями секретов (API-ключи, пароли, токены) — их вносит и меняет только владелец; выводить содержимое `.env` целиком.
- Кодовые изменения — по-прежнему только через push в `master` (автодеплой), не правками на сервере.
