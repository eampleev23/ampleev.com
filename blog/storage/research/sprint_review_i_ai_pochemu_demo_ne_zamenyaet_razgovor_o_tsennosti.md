...
- Lighting role: warm side light with richer color contrast, intentionally different from the hero portrait.
...
- `blog/public/assets/img/articles/sprint_review_ai_input.png` — regenerated by Codex on 2026-06-02 as a distinct inline candidate: two different participants, no visible window, no board/screen, no readable text; pending user visual approval.
- The two images differ materially from each other and from the Backlog Refinement hero: different people, different emotions, different background palette, different artifact visibility, and no copied window/wall setup.
