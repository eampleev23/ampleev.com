        ...
        <div class="row justify-content-center">
            <div class="col-xl-7 col-lg-8 col-md-10">
                ...
                <article class="article"><br/>
                    <h2><strong>Стек (от англ. <code>Stack</code>)</strong> и <strong>Куча (от англ. <code>Heap</code>)</strong>,
                        которые мы знаем
                        и любим</h2>
                    <p class="lead">Для того чтобы разобраться в значении параметра <code>allocs/op</code> в
                        ...
                        областях памяти, которые используются в <strong>Go</strong>-программах: <strong><i>Стек</i> (от
                            англ.
                            Stack)</strong> и
                        <strong>Куча (от англ. Heap)</strong>.</p>
                    <p class="lead">Во многих популярных языках программирования под <strong>Стеком</strong>
                        подразумевается <strong>Стек
                            вызовов функций одного Потока (от англ. Thread)</strong>. <strong>Стек вызовов</strong> -
                        это
                        Стековая
                        структура данных
                        <strong><code>LIFO (последним пришел - первым ушел)</code></strong>, в которой
                        хранится все необходимое для отработки вызванной функции (переданные аргументы,
                        локальные переменные и другие данные, необходимые для работы вызванной функции)
                        <strong>Потоком</strong>.
                        Каждый вызов функции "вталкивает" (от англ. pushes) новую функцию в <strong>Стек</strong> (по
                        сути выделяет память), а
                        каждый возврат после отработки функции "выталкивает"
                        (от англ. pops) функцию из <strong>Стека</strong> (по сути освобождает память).</p>
                    <p class="lead">Для использования Стековой памяти, мы должны иметь возможность безопасно удалить
                        все данные "втолкнутой"
                        функции в <strong>Стек</strong>,
                        когда она выполнится. Поэтому мы не можем хранить в стеке ничего, на что впоследствии
                    ...
                    <figure class="sign">
                        ...
                        <figcaption>Вид <strong>Стека</strong> вызовов через некоторое время после вызова
                            <code>println</code>
                        </figcaption>
                    </figure>

                    <p class="lead">Поскольку <strong>Потоки</strong> управляются <strong>ОС</strong>, объем памяти,
                        доступный для <strong>Стека Потоков</strong>, обычно
                        ...
                        также <strong>необходимо следить за тем, сколько данных оказывается в Стеке</strong>, особенно в
                        случае глубоко
                        вложенных рекурсивных функций. Если указатель <strong>Стека</strong> на приведенной выше
                        диаграмме пройдет через
                        <strong>Лимит Стека</strong>, программа завершится с ошибкой переполнения <strong>Стека</strong>.
                    </p>
                    <p class="lead"><strong>Куча</strong> - это более сложная область памяти. Мы по своей инициативе
                        ...
                        Куча, как
                        правило, вырастает во много раз больше, чем любой Стек потоков, и основная часть усилий по
