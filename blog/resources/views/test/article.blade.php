    ...
    <div class="container">
        <div class="row justify-content-center position-relative">
            <div class="col-lg-10 col-xl-8">
                <!-- Первый абзац статьи -->
                <!-- ----------------------------------------------------------------------------------------------------------->
                <p class="lead">Всем привет :) Это перевод, оригинал <a
                        href="https://victoriametrics.com/blog/go-http2/">тут</a>.
                </p>
                <p class="lead">Заодно возможно кто-то поделиться есть ли сейчас какой-то смысл писать переводы?
                    Нейронки отлично переводят. С точки зрения SEO как с этим дела?</p>
                <br/>
                <br/>
                <!-- ----------------------------------------------------------------------------------------------------------->
                ...
                {{--                    </div>--}}
                <img src="/assets/img/http2_golang_article_1img.png" alt="Image">
                {{--                </div>--}}
            </div>
            <!-- ----------------------------------------------------------------------------------------------------------->
            <!-- Картинка с JS -->
        ...
        <div class="row justify-content-center">
            <div class="col-xl-7 col-lg-8 col-md-10">
                ...
                <article class="article">
                    ...
                    <br/>
                    <br/>
                    <p class="lead">После того как вы освоились с net/rpc из предыдущей статьи (<a
                            href="https://victoriametrics.com/blog/go-net-rpc/index.html">From net/rpc to gRPC in Go
                            Applications</a>),
                        вероятно, будет хорошей идеей начать изучать HTTP/2, который является основой протокола gRPC.
                    </p>

                    <p class="lead">Эта статья больше посвящена теории, поэтому предупреждаем, что в ней много текста.
                        Мы
                        сосредоточимся на понимании HTTP/2, а затем вкратце коснемся его включения в Go. Так что берите
                    ...
                    <p class="lead">В HTTP/1.1 появилась конвейерная обработка, которая на бумаге выглядела как
                    ...
                    <p class="lead">
                        <br/>
                        <br/>
                        Проблема заключалась в том, что запросы отправлялись по порядку, а ответы должны
                        были возвращаться в том же порядке. Если один ответ задерживался - возможно, серверу требовалось
                        дополнительное время на его обработку, - все остальные в очереди вынуждены были ждать.</p>
                    <p class="lead">Это также происходит, если в сети происходит «заминка», которая задерживает только
                        один запрос. Весь конвейер отклика замирает, пока не пройдет этот отложенный запрос.</p>
                    <img src="/assets/img/Head-of-Line Blocking in HTTP1.1.png" alt="Image">
                    <br/>
                    <br/>
                    ...
                    <ul>
                        <li>Больше соединений означало больше ресурсов, используемых как на стороне клиента, так и на
                            стороне сервера.
                        </li>
                        <li>TCP приходится проходить через процесс рукопожатия для каждого соединения, что увеличивает
                            задержку.
                        </li>
                    </ul>
                    ...
                    <img src="/assets/img/изображение_нескольких_потоков_через_одно_соединение_http2.png"
                         alt="Иображение нескольких потоков через одно соединение при использовании http2-протокола">
                    <br/>
                    <br/>
                    ...
                    <p class="lead">На транспортном уровне TCP настаивает на доставке пакетов в порядке, необходимом для
                        прикладного уровня. Если один пакет пропадает или задерживается, TCP заставляет все остальное
                    ...
                    <br/>
                    <h3>Предисловие к соединению</h3>
                    <br/>
                    <p class="lead">
                        После завершения рукопожатия клиент отправляет нечто, называемое предисловием к соединению. Это
                        нечто начинается с очень специфической последовательности из 24 байт: PRI *
                        HTTP/2.0\r\n\r\n\nSM\r\n\r\n. Эта последовательность подтверждает, что используется протокол
                        HTTP/2. На этом этапе еще нет ни сжатия, ни кадрирования.
                    </p>
                    <br/>
                    ...
                    <br/>
                    <img src="/assets/img/Сервер_и_клиент_обмениваются_фреймами_настроек.png"
                         alt="Схема обмена сообщениями с информацией о допустимых настройках между клиентом и сервером в при использовании протокола HTTP2">
                    <br/>
                    <br/>
                    <p class="lead">Сервер понимает, какие настройки допустимы для клиента, и отвечает собственным
                        предисловием к соединению, которое включает в себя фрейм SETTINGS.</p>
