...
<head>
    ...
    <!-- Title ----------------------------------------------------------->
    <title>Эффективное управление релизами ПО на базе микросервисной архитектуры</title>
    <!-- Title ----------------------------------------------------------->
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-10 col-xl-8">
                ...
                <!-- Заголовок статьи H1-->
                <h1>Эффективное управление релизами ПО на базе микросервисной архитектуры</h1>
                <!-- Заголовок статьи H1-->
                <!-- ----------------------------------------------------------------------------------------------------------->
                <div class="d-flex align-items-center">
                    <a href="#">
...
<section class="p-0" data-reading-position>
    <div class="container">
        <div class="row justify-content-center position-relative">
            ...
            <p class="lead"><code>Микросервисная архитектура</code> - не новая технология на рынке. Без сомнения, вы уже
                слышали о
                ней, а возможно, ваша компания даже уже перешла на нее. Столь массовый переход от монолитной архитектуры
            ...
            <ol>
                <li><strong>Гибкость (Agility)</strong>. Микросервисы позволяют создавать небольшие, эффективные,
                    ...
                    нужно ждать, чтобы развернуть новую функциональность в производстве. Такая среда идеально подходит
                    для внедрения Agile-практик управления проектами. Кроме того, она значительно сокращает время выхода
                    на рынок.
                </li>
                <li><strong>Инновации</strong>. Не секрет, что процедурные и технологические изменения быстрее и легче
                    вносятся в
                    небольших масштабах. Команды могут опробовать новое и быстро понять, что работает, а что нет. Такие
...
func main() {
	...
	fmt.Printf("The value of Violet is %v\n", Violet)
                    ...
                    <p class="lead">В приведенном выше коде определены семь констант типа <strong>int</strong>. Затем
                        ...
                        следующей константы. Если запустить этот код, получим такой вывод:</p>
                    <pre class="language-go"><code>
...
const (
	...
	Baz // Baz = 6
                    ...
                    <p class="lead">Используя знак подчеркивания, мы пропустили 2 значения между <code>Bar</code> и
                        <code>Bin</code> в примере выше. Однако
                        обратите внимание, что размещение пустой строки <strong>НЕ</strong> увеличивает значение <code>iota</code>.
                        Пропуск значения <code>iota</code> возможен <strong>только</strong> при использовании
                        подчеркивания.</p>
                    ...
                    <p class="lead">Благодаря тому, что <code>iota</code> автоматически увеличивается, вы можете
                        использовать ее для
                        вычисления более сложных сценариев. Например, при работе с битовыми масками, <code>iota</code>
                        можно
                        использовать для быстрого создания правильных значений с помощью оператора битового сдвига.</p>
...
Go Version: go 1.22.0
                    ...
                    <p class="lead">Мы можем пойти еще дальше и использовать <code>iota</code>, например, для вычисления
                        объема
                        памяти. Давайте рассмотрим следующий набор констант:</p>
                    <pre class="language-go"><code>
const (
	...
	GB = 1073741824 // binary 01000000000000000000000000000000
                    ...
                    <pre class="language-go"><code>
const (
	_  = 1 << (iota * 10) // игнорирование нулевого значения
...
GB =  1073741824
                    ...
                    <p class="lead">Вы также можете объединять константы в <strong>пары</strong> на одной строке при
                        использовании <code>iota</code>.
                        Также
                        можно использовать <strong>подчеркивание</strong>, чтобы пропустить значение в этих <strong>парах</strong>.
                        Вот пример безумного
                        использования <code>iota</code>:</p>
                    <pre class="language-go"><code>
const (
	...
	ford, _
                    ...
                    <p class="lead">Как видите, при <strong>парном</strong> определении, учитывается только первое
                        значение
                        <code>iota</code> для инкремента на следующей строке:</p>
                    <pre class="language-go"><code>
tomato =  1, apple = 2
orange =  2, chevy = 3
ford   =  3
...
const (
	...
	Western
                    ...
                    <p class="lead">Теперь, если вы были внимательны, вы могли понять, что <code>iota</code> всегда
                        начинается с <code>0</code>.
                        Это означает, что значение константы <code>Adventure</code> теперь равно <code>0</code>, а не
                        <code>1</code>, как раньше. Что еще более
                        интересно, так это то, что после внесения этого изменения тесты все равно прошли. Это
                        объясняется тем, что мы стали использовать наши константы во всех наших функциях и тестах. Это
                        хорошо.</p>
                    <p class="lead">Однако реальная опасность, которая возникла в связи с этим изменением, заключается в
                        ...
                        произошло, а затем мы
                        изменили наш код, то теперь мы испортим наши данные, поскольку значение наших констант для
                        <code>Genre</code>
                        изменилось.</p>
...
ok      book    0.099s
                    ...
                    <p class="lead">Поскольку значение константы можно ошибочно изменить с помощью <code>iota</code>, не
                        ...
                        экспортированными
                        константами. Если вы используете
                        <code>iota</code> с экспортированными константами, то считайте, что вы автоматически дали
                        обещания всем
                        пользователям вашего пакета, что <strong>НИКОГДА</strong> не будете изменять значение этих
                        констант. Причина в
                        том, что вы больше не знаете, сериализовал ли кто-то из пользователей вашего пакета значение
                        вашей константы. Если в будущем вы когда-нибудь измените свою константу (намеренно или по
                        ошибке), вы испортите данные о ней.
                    </p>
...
const (
	...
	December
                    ...
                    <p class="lead">
                        Обратите внимание, что эти константы фактически экспортируются. Поэтому необходимо позаботиться
                        о том, чтобы эти значения никогда не менялись. Хотя это только мое мнение, я считаю, что
                        использование <code>iota</code> в данной ситуации неоправданно. Оно не добавляет ясности в код и
                        увеличивает
                        ненужный риск возникновения ошибки в будущем.
                    </p>
...
const (
	...
	December  Month = 12
                    ...
                    <p class="lead">Как видите, идентификатор <code>iota</code> в <strong>Go</strong> можно использовать
                        в самых разных сценариях. И
                        хотя это делает использование <code>iota</code> очень мощной концепцией, необходимо также
                        позаботиться о том,
                        чтобы будущие изменения не привели к повреждению данных. Главный вывод, который хотелось бы
                        донести в этой статье - это то, что если вы экспортируете какие-либо константы, использующие
                        <code>iota</code>,
                        вы должны обеспечить надежное тестирование этих констант, чтобы гарантировать, что никакие
                        изменения в будущем не могут быть внесены в эти константы.</p>
                    ...
                    <ol>
                        ...
                        <li><strong>Константы, определенные <code>iota</code>, будут когда-либо сериализованы и
                                десериализованы
                                (например, сохранены в файл и считаны обратно).</strong>
                        </li>
                    </ol>
                </article>
                <!-- ----------------------------------------------------------------------------------------------------------->
